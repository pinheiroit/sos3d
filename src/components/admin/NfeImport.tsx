                  ...
                  return <TableRow key={`${item.line}-${item.supplierCode}`}>
                    ...
                    <TableCell>
                      ...
                      {decision.action === "created" && <div className="mt-2 grid gap-2">
                        ...
                        <div className="grid grid-cols-2 gap-2"><Input value={decision.brand} onChange={(event) => updateDecision(index, { brand: event.target.value })} placeholder="Marca" /><Input type="number" min="0" step="0.01" value={decision.price} onChange={(event) => updateDecision(index, { price: Number(event.target.value) })} placeholder="Preço de venda (R$)" aria-label="Preço de venda em reais" /></div>
